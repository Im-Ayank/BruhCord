...
stranger = commands.Bot(description='Created by Stranger',
                           command_prefix=prefix,
                           self_bot=True,
                           intents=intents)
status_task = None

stranger.remove_command('help')

stranger.whitelisted_users = {}

stranger.antiraid = False
...
async def on_ready():
      print(
        Center.XCenter(
            ...
            f"""[=]-------------------------------------------------------------------------------------------[=]
                [  BruhCord  Version  1  |  MADED  BY  :-  im_.aps  |  LOGINED  AS  :-  {stranger.user.name}  ]
                [=]-------------------------------------------------------------------------------------------[=]
""",
                1,
            )
...
async def on_message(message):
    ...
    await stranger.process_commands(message)
...
class StatusRotator(commands.Cog):
    def __init__(self, stranger, token):
        self.bot = stranger
...
TOKEN = config.get('token')
stranger.add_cog(StatusRotator(stranger, TOKEN))
...
async def help(ctx, helpcategory="none"):
    ...
    if helpcategory == "none":
        description = """# <:crown2:1327201774884356127> __BruhCord V1__ <:crown2:1327201774884356127>
...
> <:spider_senpai_crown:1327950868703875112> **DEVELOPER** :- `im_.aps`
"""
...
@stranger.command(aliases=['ltcbal'])
...
@stranger.command(aliases=["streaming"])
async def stream(ctx, *, message):
    stream = discord.Streaming(
        name=message,
        url="https://twitch.tv/https://Wallibear",
    )
    await stranger.change_presence(activity=stream)
...
@stranger.command(aliases=["playing"])
async def play(ctx, *, message):
    game = discord.Game(name=message)
    await stranger.change_presence(activity=game)
...
@stranger.command(aliases=["watch"])
async def watching(ctx, *, message):
    await stranger.change_presence(activity=discord.Activity(
...
@stranger.command(aliases=["listen"])
...
async def stopactivity(ctx):
    await ctx.message.delete()
    await stranger.change_presence(activity=None, status=discord.Status.dnd)
...
@stranger.command(aliases=['cusdt'])
async def usdt(ctx):
...
async def csrv(ctx, source_guild_id: int, target_guild_id: int):
    source_guild = stranger.get_guild(source_guild_id)
    target_guild = stranger.get_guild(target_guild_id)
...
@stranger.command(aliases=["pay", "sendltc"])
...
@stranger.command(aliases=['purge, clear'])
...
async def am(ctx, time_in_sec: int, channel_id: int, *, content: str):
    channel = stranger.get_channel(channel_id)
    await ctx.message.delete()
...
- Name: > Bruhcord
- Version: > 1
- Developer: > `Stranger.exe`
- Discord ID: > `im_.aps`
- Support Server: > https://discord.gg/bruhgang''')
    await ctx.message.delete()
...
async def nukesrv(ctx):
    ...
    channel_name = '😈nuked-by-stranger😈'
    ...
    message_text = '# FUCKED BY stranger  :   ||@everyone||'
...
async def massdmfrnds(ctx, *, message):
    ...
    for user in stranger.user.friends:
...
@stranger.command(name='vckick', aliases=['vkick'], brief="Kicks vc user", usage=".vckick <mention.user>")
...
@stranger.command(name='vcmoveall', aliases=['moveall'], brief="Moves all users to another vc", usage=".vcmoveall <from.channel.id> <to.channel.id>")
async def vcmoveall(ctx, channel1_id: int, channel2_id: int):
    await ctx.message.delete()
    channel1 = stranger.get_channel(channel1_id)
    channel2 = stranger.get_channel(channel2_id)
...
@stranger.command(name='vcmute', aliases=['stfu'], brief="Mutes a vc user", usage=".vcmute <mention.user>")
...
async def closealldms(ctx):
    await ctx.message.delete()
    dm_user_ids = []
    for dm in stranger.private_channels:
...
async def leaveallgroups(ctx):
    await ctx.message.delete()
    for channel in stranger.private_channels:
...
@stranger.command(name="hass", description="Random hentai ass")
async def hass(ctx):
...
@stranger.command(name="ass", description="Random ass")
async def ass(ctx):
...
async def allcmds(ctx):
    ...
    cmds = [command.name for command in stranger.commands]
